// Refunds tracker — a standalone, IMMUTABLE ledger of out-of-pocket money sent back (damaged
// goods, goodwill, cash returned). Every entry carries a MANDATORY note and a deduction-cycle
// toggle (this month / carry to next month). Entries are never edited or deleted; a mistake is
// corrected by a REVERSAL row that nets it out. Distinct from the per-order "reverse a payment"
// correction on an order — this is a fresh cash outflow. Lives in Data Management.
...
export function RefundsManager() {
  ...
  const rev = useReverseRefund();
  const { can } = useAuth();
  const [open, setOpen] = useState(false);
  const [q, setQ] = useState("");
  const confirm = useConfirm();

  async function reverse(r: RefundRow) {
    if (!(await confirm({ title: `Reverse this refund of ${formatPKR(r.amount_pkr)}?`, description: "This posts a matching reversal that cancels it out. The original stays on the ledger.", destructive: true }))) return;
    try {
      await rev.mutateAsync(r);
      toast.success("Refund reversed");
    } catch (e) {
  ...
  const all = refundsQ.data ?? [];
  const reversedIds = new Set(all.map((r) => r.reverses_id).filter(Boolean) as string[]);
  const refunds = all.filter((r) => !term || (r.reason ?? "").toLowerCase().includes(term) || (r.orders?.order_no ?? "").toLowerCase().includes(term) || (REFUND_CYCLE_LABEL[r.deduction_cycle] ?? "").toLowerCase().includes(term));
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          ...
          Recorded a refund by mistake? <span className="font-medium text-foreground">Reverse</span> it — entries are never edited or deleted.
        </p>
        {can("edit") && (
                ...
                <TableRow key={r.id} className={isReversal || isReversed ? "opacity-60" : undefined}>
                  ...
                  <TableCell>
                    {isReversal ? (
                      ...
                      <Button variant="ghost" size="sm" className="h-8 gap-1.5 text-xs" onClick={() => reverse(r)} title="Post a reversal">
      ...
      <RefundDialog open={open} onOpenChange={setOpen} />
    </div>
  );
}

function RefundDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  // Reset the form to blank whenever the dialog opens (see the pattern note in DeliveriesManager).
  useEffect(() => {
    if (!open) return;
    setAmount("");
    setRefundedOn("");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" key={key}>
        ...
        <DialogFooter>
          ...
          <Button className="bg-[#cc0000] text-white hover:bg-[#a30000]" onClick={submit} disabled={create.isPending || !amount || !reason.trim()}>
