-- Add per-line product brand to invoices, snapshotted at issue time (like name/sku),
-- so it stays fixed even if the product's brand is later edited. Both invoice RPCs are
-- kept in sync (they carry shipping_type from 090084; this adds brand alongside it).
